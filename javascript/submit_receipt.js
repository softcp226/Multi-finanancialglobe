...
const submit_receipt = async (user_form) => {
  document.querySelector("#submit").innerHTML = "processing...";
  try {
    const response = await fetch(
      // "http://localhost:5000/api/user/deposit/complete",
      "https://multi-financialglobe-backend.glitch.me/api/user/deposit/complete",
      {
        method: "POST",
...
document.querySelector("#submit").onclick = () => {
  ...
  if (!receipt.value) return (receipt.style.border = "2px solid red");
  ...
  submit_receipt({
    ...
    token,
    deposit_request_id,
    transaction_hash: receipt.value,
  });
};

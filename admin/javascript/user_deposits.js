...
const handle_delete_deposit_request = async (btn, deposit_id) => {
  ...
  try {
    const response = await fetch(
      ...
      "https://multi-financialglobe-backend.glitch.me/api/admin/deposit_request/delete",
      {
        method: "DELETE",
...
const createAndAppendElement = (element) => {
  ...
  proof_evidence.innerHTML = `${element.transaction_hash||" Unavailable "}`;
...
(async () => {
  ...
  try {
    const response = await fetch(
      "https://multi-financialglobe-backend.glitch.me/api/admin/deposit_request",
